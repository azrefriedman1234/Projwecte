...
class DetailsActivity : AppCompatActivity() {
    ...
    private fun updateImageBounds() {
        ...
        if (imageBounds.width() <= 0) { 
            // אם המסך כבוי או קטן, נשתמש במימדים מלאים, אבל זה עלול לפגוע בדיוק אם זה לא תואם
            // עדיף להסתמך על הערכים השמורים מראש
    ...
    // פונקציית עזר למניעת NaN בלבד, אבל ללא ברירת מחדל אגרסיבית
    private fun cleanFloat(v: Float): Float {
        if (v.isNaN() || v.isInfinite()) return 0.0f
        // הגבלה קשוחה בין 0 ל-1 כדי למנוע קריסת FFmpeg
        if (v < 0f) return 0f
        if (v > 1f) return 1f
        return v
    }
    ...
    private fun sendData() {
        ...
        // איסוף נתונים ללא "ריכוך" - דיוק 1:1
        val rectsSnapshot = ArrayList<BlurRect>()
        for (r in b.drawingView.rects) {
            rectsSnapshot.add(BlurRect(cleanFloat(r.left), cleanFloat(r.top), cleanFloat(r.right), cleanFloat(r.bottom)))
        }

        val boundsW = if (imageBounds.width() > 0) imageBounds.width() else 1000f
        val logoW = b.ivDraggableLogo.width * savedLogoScale
        
        // חישוב רוחב יחסי מדויק
        var relW = logoW / boundsW
        if (relW.isNaN() || relW <= 0) relW = 0.2f // רק במקרה חירום
        val relativeWidthSnapshot = relW
        
        val logoRelXSnapshot = cleanFloat(savedLogoRelX)
        val logoRelYSnapshot = cleanFloat(savedLogoRelY)
            ...
            try {
                ...
                if (finalPath == null || !File(finalPath).exists()) {
                    if (targetId != 0) {
                        ...
                        for (i in 1..60) {
                            val realPath = TdLibManager.getFilePath(targetId)
                            if (realPath != null && File(realPath).exists() && File(realPath).length() > 0) { finalPath = realPath; break }; delay(1000)
                        }
                    }
                }
